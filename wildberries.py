...
class WildberriesAPI:
    ...
    async def search_products(
        ...
        Args:
            ...
            gender: Пол (мужской, женский, унисекс) для фильтрации товаров
        ...
        Примечание:
            ...
            - При противоречивых параметрах (например, low_price=30000, top_price=10000) результаты могут быть неожиданными.
              На практике оба фильтра применяются, но их взаимодействие зависит от реализации API Wildberries.
              Тесты показывают, что приоритет отдается параметру top_price.
        """
        try:
            ...
            try:
                ...
                if safe_top_price < 1:
                    ...
                    logger.warning(f"Максимальная цена скорректирована до {safe_top_price}")
                
                # Проверка на противоречивые параметры
                if safe_low_price > safe_top_price:
                    logger.warning(f"Противоречивые параметры цены: low_price ({safe_low_price}) > top_price ({safe_top_price}). Это может привести к неожиданным результатам.")
            ...
            logger.info(f"Поиск товаров по запросу '{query}' с параметрами: limit={limit}, low_price={safe_low_price}, top_price={safe_top_price}, discount={discount}, gender={gender}")
            
            # Преобразование цен в копейки для API
            price_min = safe_low_price * 100
            price_max = safe_top_price * 100
            
            # Формируем URL для поиска с учетом преобразованных цен
            url = (
            ...
            async with self._session.get(url, headers=headers, timeout=30) as response:
                ...
                # Логируем хеш ответа для отладки вместо полного текста
                import hashlib
                response_hash = hashlib.md5(text.encode()).hexdigest()
                logger.debug(f"Получен ответ (MD5: {response_hash}), длина: {len(text)} символов")
                ...
                total_products = len(data['data']['products'])
                logger.info(f"Всего найдено {total_products} товаров, обрабатываем максимум {limit}")
                ...
                for item in data['data']['products'][:limit]:
                    try:
                        ...
                        # Проверка соответствия параметрам фильтрации
                        if price < safe_low_price:
                            logger.debug(f"Товар '{name}' (ID: {product_id}) не соответствует фильтру low_price: цена {price} < {safe_low_price}")
                            continue
                            
                        if sale_price > safe_top_price:
                            logger.debug(f"Товар '{name}' (ID: {product_id}) не соответствует фильтру top_price: скидочная цена {sale_price} > {safe_top_price}")
                            continue
                        ...
                        # Генерируем список возможных URL изображений
                        image_urls = self._generate_image_urls(product_id)
                        ...
                        product = {
                            ...
                            # Используем первый URL из списка как основной
                            'image_url': image_urls[0],
                            # Включаем полный список URL изображений
            ...
            # Логирование стека исключения для отладки
            import traceback
            logger.error(f"Стек исключения: {traceback.format_exc()}")
            return []
        finally:
    ...
    def _generate_image_urls(self, product_id: str) -> List[str]:
        """
        Генерирует список возможных URL изображений для товара.
        Wildberries использует несколько CDN для изображений.
        
        Args:
            product_id: ID товара
            
        Returns:
            Список возможных URL изображений
        """
        # Разные шаблоны URL для изображений Wildberries
        templates = [
            # Основной формат URL для изображений Wildberries (wbbasket.ru)
            "https://basket-01.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-02.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-03.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-04.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-05.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-06.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-07.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-08.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-09.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-10.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-11.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-12.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-13.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-14.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            "https://basket-15.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.webp",
            
            # Альтернативные форматы (jpg вместо webp)
            "https://basket-01.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.jpg",
            "https://basket-05.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.jpg",
            "https://basket-10.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.jpg",
            "https://basket-15.wbbasket.ru/vol{vol}/part{part}/{id}/images/c516x688/1.jpg",
            
            # Старые форматы (могут работать для некоторых товаров)
            "https://basket-10.wb.ru/vol{vol}/part{part}/{id}/images/c516x688/1.jpg",
            "https://basket-05.wb.ru/vol{vol}/part{part}/{id}/images/c516x688/1.jpg"
        ]
        
        # Подготавливаем параметры для подстановки в шаблоны
        vol = product_id[:4] if len(product_id) >= 4 else product_id
        part = product_id[:6] if len(product_id) >= 6 else product_id
        
        # Генерируем все возможные URL
        urls = [template.format(vol=vol, part=part, id=product_id) for template in templates]
...
class Wildberries:
    ...
    async def search_products(
        ...
        Примечание:
            ...
            - При противоречивых параметрах (например, min_price=30000, max_price=10000) результаты могут быть неожиданными.
              Тесты показывают, что приоритет отдается параметру max_price.
        """
        try:
            # Логирование параметров запроса
            logger.info(f"Поиск товаров с параметрами: query={query}, limit={limit}, min_price={min_price}, max_price={max_price}, gender={gender}")
            
            # Проверка на противоречивые параметры цены
            if min_price is not None and max_price is not None and min_price > max_price:
                logger.warning(f"Обнаружены противоречивые параметры цены: min_price ({min_price}) > max_price ({max_price}). Результаты могут быть неожиданными.")
            
            # Модифицируем запрос с учетом пола
            if gender:
                gender_keywords = {
                    'мужской': 'мужской мужские для мужчин',
                    'женский': 'женский женские для женщин',
                    'унисекс': 'унисекс'
                }
                gender_query = gender_keywords.get(gender.lower(), '')
                if gender_query:
                    query = f"{query} {gender_query}"
                    logger.info(f"Запрос модифицирован с учетом пола: {query}")
            
            # Передаем параметры в API напрямую
            ...
            # Сортировка результатов, если указан способ сортировки и есть товары
            if sort and products:
                if sort == "price_asc":
                    ...
                    logger.info(f"Неизвестный способ сортировки: {sort}, используется сортировка по умолчанию")
            ...
            import traceback
            logger.error(f"Стек исключения: {traceback.format_exc()}")
            return []  # Возвращаем пустой список вместо вызова исключения для лучшей обработки ошибок
